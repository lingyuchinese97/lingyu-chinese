...
export function AuthField({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div
        className={cn(
          "flex min-h-[66px] items-center gap-3 rounded-md border-[1.5px] border-[#D8E5F3] bg-white px-3 py-2 transition-[border-color,box-shadow] focus-within:border-blue focus-within:[box-shadow:var(--focus-ring)] hover:border-border-strong md:min-h-[76px] md:gap-[18px] md:px-[18px] md:py-2.5",
          error && "border-red focus-within:border-red hover:border-red",
          className,
        )}
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-0.5">
          ...
          <input
            ...
            className="min-h-7 w-full border-0 bg-transparent p-0 text-base text-text outline-none placeholder:text-[#7D8FA8] focus-visible:shadow-none md:text-[17px]"
            {...props}
          />
        </div>
        {password && (
...
export function AuthTitle({ title, sub }: { title: string; sub?: string }) {
  return (
    <div className="mb-1 text-center">
      ...
      {sub && <p className="mt-1 text-[15.5px] text-text-2 md:text-lg">{sub}</p>}
    </div>
  );
}
