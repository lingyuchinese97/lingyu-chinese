/** Số liệu Trang chủ: từ vựng / câu (tổng, đã thuộc), ngữ pháp, từ mới hôm nay, bài ôn hôm nay, thẻ đến hạn, bài ôn đang dở, tiến độ bài học, thông báo chưa đọc. */
...
export const GET = api(async ({ user }) => {
  const [stats, due, active, progress, unread] = await Promise.all([
    homeStats(user.id),
    dueCount(user.id),
